--[[ Commented out functions are defined in the original and need no modification ]]
...
Domesticatable._ctor = function(self, inst)
    self.inst = inst

    -- I feel like it would be much cleaner to break domestication and obedience into two components,
	-- but they use a lot of the same hooks so I'm keeping them together for now.
...
-- Identical but we need a local copy :\
...
--Although identical, we need to refer to it again and it is local
...
local function DoDeltaDomestication(self, delta, userid)
    ...
    if old ~= self.domestication[userid] then
        self.inst:PushEvent("domesticationdelta", {old=old, new=new, userid=userid})
        return true
    end
end
...
local function UpdateDomestication(inst, self)
	...
	for userid,_ in pairs(players) do
		DoDeltaDomestication(self, TUNING.BEEFALO_DOMESTICATION_GAIN_DOMESTICATION * DECAY_TASK_PERIOD, userid)
	end
	...
	if not self.domesticationdecaypaused then
		for userid,_ in pairs(self.domestication) do
			if not players[userid] then
				DoDeltaDomestication(self, CalculateLoss(GetTime(), self.lastdomesticationgain) * DECAY_TASK_PERIOD, userid)
			end
		end
	end
...
function Domesticatable:DeltaDomestication(delta, userid)
    ...
    if DoDeltaDomestication(self, delta, userid) then
        self:CheckAndStartTask()
    end
end
...
-- Looks identical but needs to refer to our version of UpdateDomestication
...
function Domesticatable:OnSave()
    return {
		...
		-- We need to save a simple number for domestication so that people can remove the mod
